      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">{decision.date}</p>
          </div>
          <div className={`px-3 py-1 rounded-full text-sm font-bold ${decision.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {decision.passed ? 'Beslutad' : 'Avslagen'}
          </div>
        </div>
        ...
        <PartyVotes votes={decision.votes} error={decision.voteFetchError} />

        <div className="mt-6 pt-4 border-t border-gray-100 flex justify-end">
          <a 
            href={`https://www.riksdagen.se/sv/dokument-och-lagar/dokument/betankande/_${decision.id}`}
            target="_blank"
